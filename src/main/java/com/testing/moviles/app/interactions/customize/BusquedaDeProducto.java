...
public class BusquedaDeProducto implements Interaction {
    ...
    public BusquedaDeProducto(List<InfoCompra> articulos) {this.nombreArticulos = articulos;}

    public static BusquedaDeProducto aComprar(List<InfoCompra> articulos){return Tasks.instrumented(BusquedaDeProducto.class, articulos);}
    ...
    public <T extends Actor> void performAs(T actor) {
        ...
        while (bandera) {
            ...
            if (bandera) {
                actor.attemptsTo(
                        WaitUntil.the(UiPLP.LBL_NAME_ARTICULOS, WebElementStateMatchers.isPresent()).forNoMoreThan(15).seconds()
                );
                actor.attemptsTo(SwipeByCoordinates.scorll(532, 1600, 540, 505));
