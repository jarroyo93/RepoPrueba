...
public class CoordenadaArticulos {
    ...
    public static List<String> obtenerCoordenadas(Actor actor) {
        ...
        actor.attemptsTo(
                WaitUntil.the(UiPLP.BTNS_ADD, WebElementStateMatchers.isPresent()).forNoMoreThan(15).seconds()
        );
        List<String> listaCoordenadas = new ArrayList<>();
        //WebElement element = BTNS_ADD.resolveFor(actor);
        //String text = element.getText();
        // Procesa el texto según sea necesario
        //List<String> listaBotones = Arrays.asList(text.split("\\s*,\\s*"));
